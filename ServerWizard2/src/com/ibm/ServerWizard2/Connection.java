...
public class Connection {
	...
	public void writeInstanceXML(Writer out) throws Exception {
		...
		out.write("\t\t<source_target>"+source.getTarget().getName()+"</source_target>\n");
		out.write("\t\t<dest_path>"+dest.getPath()+"</dest_path>\n");
		out.write("\t\t<dest_target>"+dest.getTarget().getName()+"</dest_target>\n");
	...
	public void readInstanceXML(Element t) throws Exception {
		...
		for (int j = 0; j < attrList.getLength(); ++j) {
			...
			if (a==null) {
				throw new Exception("Attribute: "+id+" is invalid for bus "+source.getPath());
			}
