...
public class LibraryManager {
	...
	public static String getWorkingDir() {
		...
		/*
		 * String workingDir =
		 * ServerWizard2.class.getProtectionDomain().getCodeSource
		 * ().getLocation().getPath(); File f = new File(workingDir); if
		 * (f.getName().equals("main.jar")) {
		 * workingDir=f.getParentFile().getParentFile
		 * ().getParent()+System.getProperty("file.separator"); }
		 * workingDir=workingDir.replace("file:", ""); //look for drive letter
		 * colon from windows, if found, strip off leading file separator if
		 * (workingDir.charAt(2)==':') { workingDir = workingDir.substring(1); }
		 */
		// ServerWizard2.LOGGER.info("Working Dir: "+workingDir);
		return workingDir;
	}

	public void initModel(SystemModel model) throws Exception {
		for (LibraryFile libFile : files) {
			...
			if (libFile.getType() == LibraryFile.FileTypes.TARGET_INSTANCES_XML) {
				model.loadTargetInstances(new XmlHandler(), libFile.getLocalPath());
			}
		}
	}
	...
	public void init() {
		...
		try {
			...
			String swUrl = "https://raw.githubusercontent.com/nkskjames/ServerWizard2/master/";
